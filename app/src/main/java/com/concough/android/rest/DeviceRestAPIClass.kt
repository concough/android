...
//                          Log.d(TAG, resCode.toString())
        ...
        // Acquire device API
        @JvmStatic
        fun deviceState(context: Context, completion: (data: JsonObject?, error: HTTPErrorType?) -> Unit, failure: (error: NetworkErrorType?) -> Unit): Unit {
            val fullPath = UrlMakerSingleton.getInstance().getDeviceStateUrl() ?: return

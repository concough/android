...
/**
 * Created by abolfazl on 12/9/18.
 */
class UserLogRestAPIClass {
    companion object {
        const val TAG = "UserLogRestAPIClass"

        @JvmStatic
        fun syncUp(context: Context, data: ArrayList<HashMap<String, Any>>, completion: (data: JsonElement?, error: HTTPErrorType?) -> Unit, failure: (error: NetworkErrorType?) -> Unit): Unit {
            var fullPath = UrlMakerSingleton.getInstance().getUserLogSyncUpUrl() ?: return
